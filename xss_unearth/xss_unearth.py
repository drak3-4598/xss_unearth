...
if __name__ == "__main__":
    ...
    if not os.path.exists(log_file_path):
        ...
        summary_file_path = os.path.join(results_folder, f"summary_{timestamp}.log")
        ...
        with open(output_file_path, "w", encoding="utf-8") as output_file:
            ...
            if len(xss_attacks) > 0:
                ...
                summary_file.write(f"Found {len(xss_attacks)} potential XSS attacks:\n")
                for line_number, line_content in xss_attacks:
                    output_file.write(f"{line_content}\n")
                    summary_file.write(f"Row {line_number}: {line_content}\n")
                    summary_file.close()
